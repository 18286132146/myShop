...
YT.ListView = YT.extend(YT.Component, {
	...
	onRefresh : function() {
		...
		YT.log.debug("===this.refreshComplete===",this.refreshComplete);
	...
	onLoadData : function(ajax, rst, noCache) {
		if(rst){
				this.rspdata = rst.TOTAL_NUM;
		}
	/* 	this.returnTot_Num(rst); */
		...
		YT.log.debug("------onLoadData-2---", nextKey);
				...
				if (absPostX > 5 && absPostY < 5) {
					...
					if (this.pullRefresh !== false && this.$endPageY < 0) {
						...
						if (!this.finish) { // 数据加载完成
							...
							var p = Math.abs(this.$endPageY);
							if (p > 100) {
								YT.log.debug("===底部加载更多===",this.loadMoreNow);
	...
	onItemTouchEnd : function(e) {
		...
		YT.log.info(this.pullEventName);
