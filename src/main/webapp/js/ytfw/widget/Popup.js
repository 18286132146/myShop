;(function() {
	...
	var Popup = YT.Popup = {
		...
		_AppWindow :"",
		//头部菜单模板
		_popupMenuTpl :['<div class="yui-popup-mask"></div>', '<div class="yui-popup-group">'+
				'<div class="yui-popup-array">', '<em></em>', '<i></i>', '</div>', '<div class="yui-popup-menus">', '<ul>'+
				'{@each LIST as item,index}', '<li data-func="${item.func}">${item.name}</li>', '{@/each}', '</ul>'+
				'</div>', '</div>' ].join(''),
		/**
		 * 底部弹出层
		 */
		initPopupindow : function(panel,app,conf) {
			...
			var tpl_html = 
					...
					'			<div class="ui-popup-modal-button" func="${item.func}">${item.name}</div>'+
					'		{@/each}'+
					'		</div>'+
					'		<div class="ui-popup-modal-group">'+
			...
			var html = Fw.template(tpl_html,{LIST: conf.list});
			$(layer).html(html);
		...
		btnFunc : function(){
			...
			if (!YT.isEmpty(callback)) {
				Popup._AppWindow[callback] && Popup._AppWindow[callback]();
			}
