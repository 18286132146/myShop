...
(function() {
	var me = YT.Sms = {
		...
		initSmsController : function(panel, app, json) {
			...
			var url = elem.data("url") || "common/smsCodeSend"; // 请求接口链接
			if (url) {
				// 发送短信验证码的上送参数获取
				...
				var preCheck = function() {
					if (!YT.isEmpty(callback)) {// 存在回调函数
						...
						// 获取页面元素值
						return YT.Form.getFormJson(form);
					}
				}
				me.timer = me.counttime = elem.data("timer") || 60;
				me.openTimerListener(xelem, json, preCheck, function() {
								...
								if (rsp.STATUS == '1') {
									...
									if (me.relem && me.relem.length > 0) {
										...
										if (me.relem.data("flow-no-name") < 1) {
											Fw.log
													.warn("---YT.Sms---",
															"'r-send-sms' data-flow-no-name is not defined");
										}
										me.relem.find("input").val(
												rsp[me.relem
														.data("flow-no-name")
														|| "SMS_FLOW_NO"]);
									}
