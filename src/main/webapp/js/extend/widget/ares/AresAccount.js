...
define(function(require, exports) {
	...
	me.acctGroupTpl = [
			...
			'<option value="${item.ACCT_NO}" data-acctType="${item.ACCT_TYPE}">${item.ACCT_NO|fmtAcctNo,true}</option>',
	...
	me.acctCard = [
			...
			'<label class="ui-txt-info ui-padding-l45">尾号(${item.ACCT_NO|fmtAcctStop4,true})</label>',
	...
	me.initCardController = function(widget, panel, app, json) {
		...
		function buildGroup(accts, tpl) {
			var html = YT.template(tpl, {
				dataName : ele.attr("data-field-name") || "ACCT_NO",
				LIST : accts
			});
			ele.html(html);
	...
	me.initAcctController = function(widget, panel, app, json) {
		...
		function showBal(datas) {// 显示余额
			...
			var bal = datas.BALANCE_AVAILABLE || datas.AVAL_BAL
					|| datas.USA_BLE_BAL;
