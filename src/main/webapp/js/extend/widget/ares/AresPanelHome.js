/**
 * <code>
 * AresPanelHome 首页模块化加载
 * 
 * 
 * </code>
 */
define(function(require, exports) {
	var TAG = "AresPanelHome";
	...
	me.init = function(widget, panel, app, json) {
		...
		var version = widget.attr("data-version");// 版本
		var type = widget.attr("data-transType");// 类型
		if (YT.isEmpty(transUrl)) {// 请求路径为空时
			YT.loadPage(widget, tplHtmlFile, json, function() {
				app[callback] && app[callback]();
			});
		} else {// 请求路径存在时
			var url = YT.dataUrl(transUrl,true,null,true);// 格式转换路径
			YT.ajaxCashe(url, version, type, json, function(data) {// 请求接口信息数据
				if (data.STATUS == '1') {
