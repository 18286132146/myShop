...
public interface MerNeederTalkMapper extends BaseMapper<MerNeederTalk> {
    ...
    @Select(
            "select DISTINCT voicer_id as voicerId,count(*)as msgNum,img_url as headImg from (select a.*,b.*,c.img_url from mer_needer_talk a right join sender_mark b on a.voicer_id=b.sender_id left join mer_needer c on b.sender_id=c.id\n" +
...
@Select("select count(*)as msgNum from mer_needer_talk a where   a.receiver_id=#{userId} and a.create_time>=#{lastDate} and a.voicer_id=#{voicerId} \n" +
